blackmagicvideohub)
    ...
    type="pkgInDmgInZip"
    downloadURL=$(curl --compressed --location --header "Content-Type: application/json;charset=UTF-8" --header "User-Agent: Mozilla/5.0" --data '{"country": "us", "platform": "Mac OS X", "product": "Videohub"}' "$(curl -fs https://www.blackmagicdesign.com/api/support/us/downloads.json | /usr/bin/osascript -l 'JavaScript' \
    ...
    -e "update = parsed.downloads.filter((download) => download.name.match(/^Blackmagic Videohub/))[0]" \
    ...
    dmgName="Blackmagic_Videohub_${appNewVersion}.dmg"
    ;;